...
class JvmWebSocket(config: Config)(using platform: Platform, factory: WebSocketFactory) extends WebSocket {
  ...
  class Client(handler: (WebSocketEvent) => Unit, uri: String) extends WebSocketClient(new URI(uri)) {
    ...
    def onMessage(message: String): Unit = handler(OnTextMessage(JvmWebSocket.this, message))
    override def onMessage(bytes: nio.ByteBuffer): Unit = handler(OnBinaryMessage(JvmWebSocket.this, WrappedBuffer(bytes)))
